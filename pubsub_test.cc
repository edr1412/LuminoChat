    ...
    for (int i = 0; i < num_threads; ++i) {
        publisher_threads.emplace_back([&, i]() {
            for (int j = 0; j < num_messages; ++j) {
                //redis_pub_sub.publish(channel_prefix + std::to_string(i),std::to_string(j));
                if(!redis_pub_sub.publish(channel_prefix + std::to_string(i), std::to_string(j)))
                {
                    //abort();
...
void messageHandler2(const std::string &channel, const std::string &message)
{
    printf("[%s] %s\n", channel.c_str(), message.c_str());
}
...
{
    RedisPubSub redis_pub_sub("localhost", 6379, messageHandler);
    RedisPubSub redis_pub_sub2("172.16.61.1", 6379, messageHandler2);

    redis_pub_sub.subscribe("channel_1");
    redis_pub_sub2.subscribe("channel_2");

    redis_pub_sub.publish("channel_1", "111");
    redis_pub_sub2.publish("channel_2", "222");

    std::thread publisher([&]()
                            {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        redis_pub_sub.publish("channel_1", "Hello, World!");
        redis_pub_sub2.publish("channel_2", "Hello, World!"); });
    redis_pub_sub.publish("channel_1", "222");
    publisher.join();
    redis_pub_sub.publish("channel_1", "333");
    redis_pub_sub2.publish("channel_2", "333");

    redis_pub_sub2.unsubscribe("channel_2");
